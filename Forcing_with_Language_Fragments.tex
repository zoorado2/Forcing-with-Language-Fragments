...
Note that $\neg$ is $\Delta_0$-definable with a single parameter $\ulcorner \neg \urcorner$, which we assume is in every sufficiently transitive model of a sufficiently strong set theory. So the definition of $\neg$ is absolute for all such models of set theory.
\end{rem}
...
\begin{enumerate}[label=(\alph*)]
    ...
    \item\label{342} $(A; \in)$ is a model of a sufficiently strong set theory,
