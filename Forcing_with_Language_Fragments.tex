...
\begin{defi}
Let $\mathbb{P} = (P, \leq_{\mathbb{P}})$ and $\mathbb{Q} = (Q, \leq_{\mathbb{Q}})$ be two forcing notions. An embedding $\pi$ from $\mathbb{P}$ into $\mathbb{Q}$ is \emph{complete} iff for every maximal antichain $A$ of $\mathbb{P}$, $$\{\pi(p) : p \in A\}$$ is a maximal antichain of $\mathbb{Q}$.
\end{defi}
...
For any forcing notion $\mathbb{P}$, $w(w(\mathbb{P})) = w(\mathbb{P})$.
\end{fact}
...
\begin{defi}
Given two forcing notions $\mathbb{P}$ and $\mathbb{Q}$, we say $\mathbb{P} \lessdot \mathbb{Q}$ iff there is a complete embedding from $w(\mathbb{P})$ into $w(\mathbb{Q})$.
\end{defi}

\begin{fact}\label{forpre}
The relation $\lessdot$ is a preorder on the class of all forcing notions.
\end{fact}

\begin{defi}\label{comdef}
Forcing notions $\mathbb{P}$ and $\mathbb{Q}$ are \emph{forcing equivalent} iff $\mathbb{P} \lessdot \mathbb{Q}$ and $\mathbb{Q} \lessdot \mathbb{P}$.
\end{defi}

\begin{rem}\label{sepeq}
By Definition \ref{comdef}, for any forcing notion $\mathbb{P}$, $\mathbb{P}$ and $w(\mathbb{P})$ are forcing equivalent.
\end{rem}
...
Let $\mathbb{P}$ and $\mathbb{Q}$ be two forcing notions. By Remark \ref{sepeq} and Fact \ref{defor}, if there is a dense weak embedding from $\mathbb{P}$ into $\mathbb{Q}$, then $\mathbb{P}$ and $\mathbb{Q}$ are forcing equivalent.
\end{rem}
...
From the perspective of forcing extensions, Theorems \ref{genericmodels} and \ref{revgenmodels} tell us that 
\begin{enumerate}
...
\begin{enumerate}[label=(\arabic*)]
    ...
    \item Is it consistent that there exists a first-order $\Pi_2$ theory $T$ with a model $M$ in some outer model of $V$ such that for no $\Pi_2$ TCI $\mathfrak{T}$ is $M$ isomorphic to a $V$-generic model of $\mathfrak{T}$?
\end{enumerate}
\end{ques}
...
\begin{enumerate}[label=(\arabic*)]
    ...
    \item Is it always true that
    \begin{align*}
        & \bigcup \{V[\mathcal{M}] : \mathcal{M} \models^* \mathfrak{T} \text{ in an outer model of } V\} \\
        & =  \bigcup \{V[g] : g \text{ is } \mathbb{P}(\mathfrak{T}) \text{-generic over } V\} \text{?}
    \end{align*}
\end{enumerate}
\end{ques}
...
\begin{enumerate}[label=(\alph*)]
    \item $\mathcal{C} \subsetneq \{\mathfrak{T} : \mathfrak{T} i\text{ is a } \Pi_2 \text{ TCI}\}$, and
...
Is there a ``naturally definable'' class $\mathcal{C}$ such that 
\begin{enumerate}[label=(\alph*)]
    \item $\mathcal{C} \supsetneq \{\mathfrak{T} : \mathfrak{T} i\text{ is a } \Pi_2 \text{ TCI}\}$, and
