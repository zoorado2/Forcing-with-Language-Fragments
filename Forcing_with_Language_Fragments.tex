...
\begin{itemize}
    ...
    \begin{itemize}[label=$\circ$]
        ...
        \item $z$ is a variable not bound in $\phi$, and
        \item $p$ is either a constant or a variable not bound in $\phi$,
    \end{itemize} 
    then $$\ulcorner \forall z \ ((z \in p \wedge \text{``}\emptyset \neq p \subset \mathcal{L} \wedge |p| < \omega \text{''}) \implies \phi) \urcorner \in \mathcal{L}',$$
...
\begin{enumerate}[label=(\Roman*)]
    \item The statement $$\text{``}\emptyset \neq p \subset \mathcal{L} \wedge |p| < \omega \text{''}$$ can be expressed as a formula in $\mathcal{L}^{*}_{0, \mathfrak{A}}$.
...
Due to the nature of conversion algorithms such as $\mathsf{P_1}$, whenever $\phi$ is a safe $\mathcal{L}^{*}_{\mathfrak{A}}$-$\Delta_0$ formula, $\mathsf{DNF}_1(\phi)$ is a sentence in $\mathcal{D}$ logically equivalent to $\phi$. 
...
\begin{quote}
    ...
    \begin{enumerate}[label=(\arabic*)]
        ...
        \item For each $\phi'$-max peeling $\varphi$ containing $\ulcorner E \urcorner$ (the order does not matter since distinct $\phi'$-max peelings are disjoint):
        \begin{enumerate}[label=(F\arabic*), leftmargin=30pt]
            \item Necessarily, for some $p$ and $\varphi'$,
                \begin{equation*}
                    \varphi = \ulcorner \forall z \ ((z \in p \wedge \text{``}\emptyset \neq p \subset \mathcal{L} \wedge |p| < \omega \text{''}) \implies \varphi') \urcorner \text{.}
                \end{equation*}
...
Obviously, $\mathsf{P_W}$ always terminates and returns a $\mathcal{L}^{*}_{\mathfrak{A}}$-$\Delta_0$ formula. Furthermore, routine code tracing allows us to ascertain that the output of $\mathsf{P_W}$ is always logically equivalent to its input. In other words, 
\begin{equation*}
        ...
        \begin{enumerate}[label=(F\arabic*), leftmargin=30pt]
            \item Necessarily, for some $p$ and $\varphi'$,
                ...
                \begin{enumerate}[label=Case \arabic*:, leftmargin=50pt]
                    ...
                    \item\label{f1c2} $\neg \varphi$ is a conjunct of a disjunct of $\phi$'.
                \end{enumerate}
                For our next step, we consider these two cases separately.
            
            \item\label{step3} \underline{In the event of \ref{f1c1}}
            
            If $p$ is a not a free variable, then it must be a constant symbol. In this case, we check if 
            \begin{equation*}
                \mathfrak{A} \models \text{``}\emptyset \neq p \subset \mathcal{L} \wedge |p| < \omega \text{''.}
            \end{equation*}
            If so, replace $\varphi$ with 
            \begin{equation*}
                \ulcorner \bigwedge \{\varphi'[z \mapsto a] : a \in p\} \urcorner
            \end{equation*}
            in $\phi'$. If not, replace $\varphi$ with $\ulcorner \text{``} \emptyset = \emptyset \text{''} \urcorner$ in $\phi'$. 
        
            Next, consider the case where $p$ is a free variable. In this case, we check if
            \begin{equation*}
                \mathfrak{A} \models \text{``}\emptyset \neq \nu(p) \subset \mathcal{L} \wedge |\nu(p)| < \omega \text{''.}
            \end{equation*}
            If so, replace $\varphi$ with 
            \begin{equation*}
            ...
            \begin{equation*}
                \ulcorner \neg \text{``}\emptyset \neq p \subset \mathcal{L} \wedge |p| < \omega \text{''} \urcorner
            \end{equation*}
            in $\phi'$.
            ...
            If so, replace $\varphi$ with 
            \begin{equation*}
                \ulcorner \bigvee \{\neg \varphi'[z \mapsto a] : a \in p\} \urcorner
            \end{equation*}
...
with 
\begin{equation*}
    \ulcorner (\varphi \wedge (x \in \mathcal{L} \implies E(\neg x))) \urcorner
\end{equation*}
in $\phi$.
\end{defi}

\begin{rem}\label{shcs}
In the definition of $\mathsf{check}$, $E(\neg x)$ is a shorthand for $$\exists y \ (y = \neg (x) \wedge E(y)),$$ where $\neg (\cdot)$ is the negation function on $V$, so that ``$y = \neg (x)$'' is expressible as a ($\Delta_0$, no less) formula in $\mathcal{L}^{*}_{0, \mathfrak{A}}$. As a consequence, if $\ulcorner \exists x_1 ... \exists x_n \ \phi \urcorner$ is a $\mathcal{L}^{*}_{\mathfrak{A}}$-$\Sigma_1$ formula, then $$\ulcorner \exists x_1 ... \exists x_n \ \mathsf{pos}(\mathsf{check}(\phi)) \urcorner$$ is logically equivalent to a $\mathcal{L}^{*}_{\mathfrak{A}}$-$\Sigma_1$ formula over any structure with a non-empty base set (e.g. any expansion of $\mathfrak{A}$). Without loss of generality, we shall always assume formulas of this form to be $\mathcal{L}^{*}_{\mathfrak{A}}$-$\Sigma_1$.
\end{rem}
...
\begin{enumerate}[label=Case \arabic*:, leftmargin=50pt]
    ...
    \begin{align*}
        \phi'' := \ & \mathsf{check}(\phi^*) \\
        \phi''' := \ & \ulcorner \bigvee \{\mathsf{check}(\mathsf{pos}(\psi)) : \psi \text{ is a disjunct of } \phi'\} \urcorner \text{.}
    \end{align*}
    Clearly, 
    \begin{equation*}
        \phi''' = \mathsf{check}(\mathsf{pos}(\phi')) = \mathsf{pos}(\mathsf{check}(\phi')) \text{,}
    \end{equation*}
    from which, citing Proposition \ref{prop321} and Fact \ref{fact330}, we can conclude
    \begin{equation}\label{eq32f}
        \phi''' = \mathsf{pos}(\mathsf{DNF}(\phi'', \nu)) = \mathsf{DNF}(\mathsf{pos}(\phi''), \nu) \text{.}
    \end{equation}
    Finally, set
    \begin{equation*}
        \phi^{\dagger} := \ulcorner \exists x_1 \dots \exists x_n \ \mathsf{pos}(\phi'') \urcorner \text{.}
    \end{equation*}
    It is imperative to highlight that both $\phi''$ and $\phi^{\dagger}$ depend on $\phi^*$, and thus on $\phi$, but not on any $\mathfrak{A}$-valuation.
    ...
    \begin{itemize}
        ...
        \item $\varphi^*$ is a disjunct of $\mathsf{DNF}(\mathsf{pos}(\phi''), \nu')$, and
        \item $\Sigma' \models^{*}_{\mathfrak{A}, \nu'} \varphi^*$.
    \end{itemize}
    ...
    Claim \ref{claim335} informs us that the set $$D_2 := \{q \in P : q \models^{*}_{\mathfrak{A}} \phi^{\dagger}\}$$ is dense below $p$ in $\mathbb{P}$. With reference to Remarks \ref{def2} and \ref{shcs}, $D_2$ is $\Sigma_1$-definable in the language associated with $\mathfrak{A}$, since
    \begin{itemize}
        \item $\mathbb{P}$ is $\Sigma_1$-definable in the language associated with $\mathfrak{A}$,
        \item $\phi^{\dagger}$ is $\mathcal{L}^{*}_{\mathfrak{A}}$-$\Sigma_1$, and
...
\begin{enumerate}[label=(S\arabic*)$_{\lambda}$, leftmargin=40pt]
    ...
    $\begin{aligned}[t]
        ...
        & \forall e \ ((e \in p \wedge \text{``}\emptyset \neq p \subset \mathcal{L} \wedge |p| < \omega \text{''}) \implies E(e)))) \urcorner.
    \end{aligned}$
\end{enumerate}
\end{defi}
