...
In the definition of $\mathsf{check}$, $E(\neg x)$ is a shorthand for both $$\exists y \ (y = \neg (x) \wedge E(y))$$ and $$\forall y \ (y = \neg (x) \implies E(y)),$$ where $\neg (\cdot)$ is the negation function on $V$, so that ``$y = \neg (x)$'' is expressible as a ($\Delta_0$, no less) formula in $\mathcal{L}^{*}_{0, \mathfrak{A}}$. As a consequence, if $\ulcorner \exists x_1 ... \exists x_n \ \phi \urcorner$ is a $\mathcal{L}^{*}_{\mathfrak{A}}$-$\Sigma_1$ formula with $\phi \in ran(\mathsf{WNF})$, then we can show by induction on the complexity of $\phi$, noting
\begin{itemize}
    \item the recursive structure of $\mathsf{P_W}$, 
    \item how $\mathsf{DNF_1}$ is used in $\mathsf{P_W}$, 
    \item Remark \ref{rem322}, and
    \item $\mathfrak{A}$ is a model of a sufficiently strong set theory,
\end{itemize}
that there exists a $\mathcal{L}^{*}_{\mathfrak{A}}$-$\Sigma_1$ formula $\varphi$ satisfying $$X \models^*_{\mathfrak{A}} \ulcorner \exists x_1 ... \exists x_n \ \mathsf{check}(\mathsf{pos}(\phi)) \urcorner \iff X \models^*_{\mathfrak{A}} \varphi$$ for every set $X$ found in any outer model of $V$. We can thus without loss of generality, assume formulas of the form $$\ulcorner \exists x_1 ... \exists x_n \ \mathsf{check}(\mathsf{pos}(\phi)) \urcorner$$ for some $n < \omega$ and $\phi \in ran(\mathsf{WNF})$, to be $\mathcal{L}^{*}_{\mathfrak{A}}$-$\Sigma_1$.
\end{rem}
...
\begin{enumerate}[label=Case \arabic*:, leftmargin=50pt]
    ...
    \begin{itemize}
        ...
        \item we can safely assume $\phi^{\dagger}$ to be $\mathcal{L}^{*}_{\mathfrak{A}}$-$\Sigma_1$, and
