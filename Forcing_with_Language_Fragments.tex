...
\begin{itemize}
    \item $\pi_{0i}(N) = N_i$,
    \item ${^{\omega_1^{\bar{M_i}}}{N_i}} \subset N_i$,
...
\begin{itemize}
    \item $W \models ``H(\omega_2)^V$ is countable",
    \item $V \models ``\mathrm{NS}_{\omega_1}$ is saturated'', and
    \item $\omega_1^W \in V$.
\end{itemize}
If 
\begin{itemize}
    \item $\lambda \leq \kappa \leq \omega_1^W$,
...
\end{itemize}
then $\pi_{\lambda \kappa}$ lifts to a generic ultrapower map $\pi^{+}_{\lambda \kappa} : V \longrightarrow M^+$, for some well-founded $M^+ \subset W$ with $M_{\kappa} \subset M^+$.
\end{lem}
\begin{proof}
