...
\begin{enumerate}[label=(\arabic*)]
    ...
    \item When we say ``a sufficiently strong set theory'', we mean to emphasise the low strength of the theory. In such instances, well-known set theories far weaker than $\mathsf{ZFC}$, such as $\mathsf{Z^-}$ and $\mathsf{KP\omega}$, typically work.
\end{enumerate}
\end{con}
...
\begin{itemize}
    \item $X$ be a set,
    \item $\mathfrak{A}$ be a structure with base set $A$ in a possibly expanded language of set theory, and
    \item $n < \omega$. 
\end{itemize} 
...
\begin{itemize}
    \item $X$ be a class, 
    \item $\mathfrak{A}$ be a structure with base class $A$ and signature $\sigma$ such that 
    \begin{itemize}[label=$\circ$]
...
If $\mathfrak{A} = (A; \in)$, we may identify $\mathfrak{A}$ with $A$ and say $X$ is $\Sigma_n$\emph{-definable} (resp. $\Pi_n$\emph{-definable} and $\Delta_n$\emph{-definable}) \emph{in} $A$ iff $X$ is $\Sigma_n$-definable (resp. $\Pi_n$-definable and $\Delta_n$-definable) in $\mathfrak{A}$. 

We say $X$ is $\Sigma_n$\emph{-definable} (resp. $\Pi_n$\emph{-definable} and $\Delta_n$\emph{-definable}) iff $X$ is $\Sigma_n$-definable (resp. $\Pi_n$-definable and $\Delta_n$-definable) in $\mathfrak{A}'$ for every structure $\mathfrak{A}'$ satisfying
\begin{itemize}[label=$\circ$]
...
Let $\mathcal{L}^{*}_{0, \mathfrak{A}}$ consist of all $\Delta_0$ formulas (as per Definition \ref{def27}) in the language associated with $\mathfrak{A}$. Obviously, $\mathcal{L}^{*}_{0, \mathfrak{A}} \subset \mathcal{L}^{*}_{\mathfrak{A}}$.
...
\begin{enumerate}[label=(\Roman*)]
    \item\label{s31I} The statement $$\text{``}\emptyset \neq p \subset \mathcal{L} \wedge |p| < \omega \text{''}$$ can be expressed as a $\Delta_1$ formula (in accordance with Definition \ref{def27}) in the language associated with $\mathfrak{A}$.
...
\begin{enumerate}[label=Case \arabic*:, leftmargin=50pt]
    ...
    Claim \ref{claim335} informs us that the set $$D_2 := \{q \in P : q \models^{*}_{\mathfrak{A}} \phi^{\dagger}\}$$ is dense below $p$ in $\mathbb{P}$. With reference to \ref{s31I} and Remarks \ref{def2} and \ref{shcs}, $D_2$ is $\Sigma_1$-definable in the language associated with $\mathfrak{A}$, since
    \begin{itemize}
        ...
        \item the act of replacing every subformula of $\phi^{\dagger}$ of the form $\ulcorner E(x) \urcorner$ with $\ulcorner x \in q \urcorner$ does not alter the quantification structure of $\phi^{\dagger}$, and
        \item after replacing every subformula of $\phi^{\dagger}$ of the form $\ulcorner E(x) \urcorner$ with $\ulcorner x \in q \urcorner$, one obtains a formula which is provably equivalent over $\mathsf{ZFC}$ to some $\Sigma_1$ formula (in the sense of Definition \ref{def27}) $\varphi^{\dagger}$ over the language associated with $\mathfrak{A}$.
    \end{itemize}
