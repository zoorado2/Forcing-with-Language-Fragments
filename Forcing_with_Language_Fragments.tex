...
\begin{enumerate}[label=(\arabic*)]
    ...
    \item\label{cor5492} There is a procedure to uniformly decide in $V$, whether every model of any given $\Pi_2$ TCI $\mathfrak{T}$ is definable in $V$ using parameters in $$trcl(\mathfrak{T}) \cup H(\omega) \cup \theta \text{,}$$ where $$\theta := max\{|trcl(\mathfrak{T})|^+, \omega_1\} \text{.}$$
\end{enumerate}
\end{cor}
