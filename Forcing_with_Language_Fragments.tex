...
\end{equation}
we mean to emphasise the low strength of the set theory $\mathfrak{A}$ satisfies. To be more concrete, we may expand \textit{Kripke-Platek set theory}, denoted $\mathsf{KP}$, to $\mathsf{KP}(\mathfrak{A})$ so as to fit the signature of $\mathfrak{A}$. This can be done as follows:
\begin{itemize}
...
\begin{quote}
    $\text{``}\mathfrak{A} \text{ is a model of some first-order theory } T \text{ such that}$ 
    \begin{itemize}
        \item $\mathsf{KP}(\mathfrak{A}) \subset T$, and
...
\begin{quote}
    ...
    \begin{enumerate}[label=(\arabic*)]
        ...
        \begin{enumerate}[label=(F\arabic*), leftmargin=30pt]
            ...
            \end{equation*}
            If so, then $\mathfrak{A}$ being a model of a sufficiently strong set theory means that 
            \begin{itemize}
