...
\end{itemize}
A $\phi$\emph{-max peeling} is a maximal member of $\mathrm{QA}(\phi)$ with respect to set inclusion.
\end{defi}
...
Let $\mathsf{DNF}$ be the function with domain
\begin{equation*}
    \mathrm{FV} := \{\phi: \phi \text{ is a } \mathcal{L}^{*}_{\mathfrak{A}}\text{-}\Delta_0 \text{ formula}\} \times \{\nu : \nu \text{ is a } \mathfrak{A}\text{-valuation}\} \text{,}
\end{equation*} 
...
\begin{quote}
    ...
    \begin{enumerate}[label=(\arabic*)]
        ...
        \begin{enumerate}[label=(F\arabic*), leftmargin=30pt]
            \item Necessarily, for some $p$ and $\varphi'$,
                \begin{equation*}
                    \varphi = \ulcorner \forall z \ ((z \in p \wedge \text{``}\emptyset \neq p \subset \mathcal{L} \wedge |p| < \omega \text{''}) \implies \varphi') \urcorner
                \end{equation*}
    ...
    \end{enumerate}
\end{quote}
That $\mathsf{P_0}$ always terminates and returns a safe $\mathcal{L}^{*}_{\mathfrak{A}}$-$\Delta_0$ formula can be easily verified. Furthermore, since $\mathsf{P_0}$ on input $(\phi, \nu)$ executes a function application of $\mathsf{DNF}_1$ on a possibly updated version of $\phi$ before termination, the end result must be a member of $\mathcal{D}$. We can thus conclude that $\mathsf{DNF}$ is a function from $\mathrm{FV}$ into $\mathcal{D}$.

The next proposition can be checked by routine, if tedious, applications of mathematical induction.
...
\begin{enumerate}[label=Case \arabic*:, leftmargin=50pt]
    ...
    \end{itemize}
    According to $\ref{p3202}$ of Proposition \ref{p320}, it must be the case that $\Sigma \models^{*}_{\mathfrak{A}, \nu} \phi'$, where
    \begin{equation*}
        \phi' := \mathsf{DNF}(\phi^*, \nu) \text{.}
    \end{equation*}
    There also ought to exist some $\varphi$ for which
    \begin{itemize}
    ...
    \end{equation}
    Through a routine unfurling of definitions, one can check that $$q \models^{*}_{\mathfrak{A}} \nu^*(\mathsf{check}(\mathsf{pos}(\varphi)))$$ indeed follows from (\ref{eq32}) and (\ref{eq33}). As $\nu^*(\mathsf{check}(\mathsf{pos}(\varphi)))$ logically implies the $\mathcal{L}^{*}_{\mathfrak{A}}$ sentence $$\phi'' := \ulcorner \exists x_1 ... \exists x_n \ (\bigvee \{\mathsf{check}(\mathsf{pos}(\psi)) : \psi \text{ is a disjunct of } \phi'\}) \urcorner,$$ we have $q \models^{*}_{\mathfrak{A}} \phi''$ as well.
...
\begin{enumerate}[label=(Q\arabic*)]
    ...
    \begin{itemize}
        ...
        \item $(\mathfrak{A}', \mathbb{P}')$ is good for $\mathcal{L}'$,
        \item $W$ is a weak outer model of $V$,
        \item $g \in W \cap \mathcal{P}(P')$ is a $\mathbb{P}'$-$\Sigma_1$-generic filter over $\mathfrak{A}'$,
