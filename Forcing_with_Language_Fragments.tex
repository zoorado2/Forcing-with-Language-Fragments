...
\begin{defi}
Let $\mathfrak{A}$ be a structure in a possibly expanded language of set theory with signature $\sigma$. When we say 
\begin{equation}\label{eq11}
    \text{``}\mathfrak{A} \text{ is a model of a sufficiently strong set theory'',} 
\end{equation}
we mean to emphasise the low strength of the set theory $\mathfrak{A}$ satisfies. To be more concrete, we may expand \textit{Kripke-Platek set theory}, denoted $\mathsf{KP}$, to fit the signature of $\mathfrak{A}$ as follows:
\begin{itemize}
    \item index the Axiom Schema of Induction by the set of all formulas over $\sigma$,
    \item index both the Axiom Schema of $\Delta_0$-separation and the Axiom Schema of $\Delta_0$-collection by the set of all $\Delta_0$ formulas (as per Definition \ref{def27}) over $\sigma$,
    \item leave all the other axioms of $\mathsf{KP}$ untouched.
\end{itemize}
Then one may always assume (\ref{eq11}) to mean 
\begin{equation*}
    \text{``}\mathfrak{A} \text{ is a model of the expanded version of } \mathsf{KP} \text{''.} 
\end{equation*}
\end{defi}
...
\begin{enumerate}[label=(\Roman*)]
    \item\label{s31I} The statement $$\text{``}\emptyset \neq p \subset \mathcal{L} \wedge |p| < \omega \text{''}$$ can be expressed as a $\Delta_1$ formula (going by Definition \ref{def27}) in the language associated with $\mathfrak{A}$. In fact, said statement can be expressed as a $\Delta_0$ formula in the language of set theory if 
    \begin{equation*}
        \mathfrak{A} \models \text{``there are no infinite sets''.}
    \end{equation*}
...
\begin{enumerate}[label=Case \arabic*:, leftmargin=50pt]
    ...
    \begin{itemize}
        ...
        \end{itemize}
        and thus,
        \item after replacing every subformula of $\phi^{\dagger}$ of the form $\ulcorner E(x) \urcorner$ with $\ulcorner x \in q \urcorner$, one obtains a formula which is provably equivalent over $\mathsf{ZFC}$ to some $\Sigma_1$ formula $\varphi^{\dagger}$ in the language associated with $\mathfrak{A}$.
    \end{itemize}
...
\begin{enumerate}[label=(\arabic*)]
    ...
    \end{itemize}
    Now, convert $\phi'$ to a logically equivalent formula $\phi^*$ (which exists because $\mathfrak{A}_{\mathfrak{T}}$ is a model of a sufficiently strong set theory) in prenex normal form, through an application of the standard conversion algorithm. This algorithm preserves the quantification structure of $\phi'$ --- so that $\phi^*$ and $\phi$ have the same quantification structure --- whenever $\phi$ is in prenex normal form.
