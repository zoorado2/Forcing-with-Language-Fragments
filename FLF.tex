...
\subsection{Content Structure and Dependencies}
...
\begin{defi}
If $\mathbb{B} = (B, \mathbb{0}, \mathbb{1}, \leq_{\mathbb{B}}, \wedge_{\mathbb{B}}, \vee_{\mathbb{B}}, \sim_{\mathbb{B}})$ is a boolean algebra, then define
\begin{align*}
    B^+ := \ & B \setminus \{\mathbb{0}\} \\
    \leq_{\mathbb{B}^+} \ := \ & \leq_{\mathbb{B}} \cap \ (B^+ \times B^+) \\
    \wedge_{\mathbb{B}^+} := \ & \wedge_{\mathbb{B}} \cap \ (B^+ \times B^+) \\
    \vee_{\mathbb{B}^+} := \ & \vee_{\mathbb{B}} \cap \ (B^+ \times B^+) \\
    \sim_{\mathbb{B}^+} \ := \ & \sim_{\mathbb{B}} \cap \ (B^+ \times B^+) \\
    \mathbb{B}^+ := \ & (B^+, \mathbb{1}, \leq_{\mathbb{B}^+}, \wedge_{\mathbb{B}^+}, \vee_{\mathbb{B}^+}, \sim_{\mathbb{B}^+}) \\
    \mathbb{B}^+_0 := \ & (B^+, \leq_{\mathbb{B}^+}).
\end{align*}
\end{defi}
...
\begin{defi}
Let $\mathbb{P} = (P, \leq_{\mathbb{P}})$ and $\mathbb{Q} = (Q, \leq_{\mathbb{Q}})$ be two forcing notions. We say $\pi$ is an \emph{embedding from} $\mathbb{P}$ \emph{into} $\mathbb{Q}$ iff $\pi$ is an injective function from $P$ into $Q$ satisfying
\begin{itemize}
    \item $p_1 \leq_{\mathbb{P}} p_2 \iff \pi(p_1) \leq_{\mathbb{Q}} \pi(p_2)$, and
    \item $p_1 \not \! \! \bot_{\mathbb{P}} \ p_2 \implies \pi(p_1) \not \! \! \bot_{\mathbb{Q}} \ \pi(p_2)$.
\end{itemize}
\end{defi}

\begin{defi}
Let $\mathbb{P} = (P, \leq_{\mathbb{P}})$ and $\mathbb{Q} = (Q, \leq_{\mathbb{Q}})$ be two forcing notions. An embedding $\pi$ from $\mathbb{P}$ into $\mathbb{Q}$ is \emph{dense} iff $ran(\pi)$ is dense in $\mathbb{Q}$.
\end{defi}

\begin{fact}\label{defor}
Let $\mathbb{P}$ and $\mathbb{Q}$ be two forcing notions. If there is a dense embedding from $\mathbb{P}$ into $\mathbb{Q}$, then $\mathbb{P}$ and $\mathbb{Q}$ are forcing equivalent.
\end{fact}

\begin{defi}
Let $\mathbb{P} = (P, \leq_{\mathbb{P}})$ be a forcing notion. Define \begin{align*}
...
For any forcing notion $\mathbb{P}$, 
\begin{enumerate}[label=(\arabic*)]
    \item $w(w(\mathbb{P})) = w(\mathbb{P})$, and
    \item $\mathbb{P}$ is forcing equivalent to $w(\mathbb{P})$.
\end{enumerate}
\end{fact}

\begin{defi}
A forcing notion $\mathbb{P}$ is \emph{separative} iff $w(\mathbb{P}) = \mathbb{P}$.
\end{defi}

\begin{fact}\label{boolsep}
If $\mathbb{B}$ is a boolean algebra, then $\mathbb{B}^+_0$ is a separative forcing notion.
\end{fact}

\begin{fact}\label{boolcom}
For any forcing notion $\mathbb{P}$, there is a unique (up to isomorphism) complete boolean algebra $\mathbb{B}$ such that a dense embedding exists from $\mathbb{P}$ into $\mathbb{B}^+_0$.
\end{fact}

\begin{rem}\label{sepfor}
By Facts \ref{defor} and \ref{boolcom}, every forcing extension of $V$ is a $\mathbb{B}^+_0$-generic extension for some complete boolean algebra $\mathbb{B}$.  
\end{rem}

\begin{defi}
Let $\mathbb{P} = (P, \leq_{\mathbb{P}})$ and $\mathbb{Q} = (Q, \leq_{\mathbb{Q}})$ be two forcing notions. We say $\pi$ is a \emph{weak embedding from} $\mathbb{P}$ \emph{into} $\mathbb{Q}$ iff $\pi$ is an embedding from $w(\mathbb{P})$ into $w(\mathbb{Q})$.
\end{defi}

\begin{defi}
Let $\mathbb{P} = (P, \leq_{\mathbb{P}})$ and $\mathbb{Q} = (Q, \leq_{\mathbb{Q}})$ be two forcing notions. A weak embedding $\pi$ from $\mathbb{P}$ into $\mathbb{Q}$ is \emph{dense} iff $\pi$ is dense as an embedding from $w(\mathbb{P})$ into $w(\mathbb{Q})$.
\end{defi}

\begin{rem}\label{dwefor}
Let $\mathbb{P}$ and $\mathbb{Q}$ be two forcing notions. By Facts \ref{defor} and \ref{wfixedp}, if there is a dense weak embedding from $\mathbb{P}$ into $\mathbb{Q}$, then $\mathbb{P}$ and $\mathbb{Q}$ are forcing equivalent.
\end{rem}
...
Henceforth, we will look into the extent to which generic models of $\Pi_2$ TCIs are abundant. We begin with two of the main theorems in this subsection, thereby establishing the connection between $\Pi_2$ TCIs and forcing extensions.
...
Let $\mathbb{B} = (B, \mathbb{0}, \mathbb{1}, \leq_{\mathbb{B}}, \wedge_{\mathbb{B}}, \vee_{\mathbb{B}}, \sim_{\mathbb{B}})$ be a boolean algebra. Then there is a consistent $\Pi_2$ TCI $\mathfrak{T}$ and a dense weak embedding from $\mathbb{B}^+_0$ into $\mathbb{P}(\mathfrak{T})$.
\end{thm}

\begin{proof}
Choose
\begin{itemize}
    \item $\dot{\mathbb{1}}$ to be a constant symbol, and
    \item $\dot{\leq}$, $\dot{\sim}$, $\dot{G}$ and $\dot{\mathcal{U}}$ to be distinct relation symbols of arities $2$, $2$, $1$ and $1$ respectively.
\end{itemize}
Set $\sigma$ to be $\{\dot{\mathbb{1}}, \dot{\leq}, \dot{\sim}, \dot{G}\}$. We define $\vartheta$ on $\{\dot{\mathcal{U}}, \dot{\mathbb{1}}, \dot{\leq}, \dot{\sim}, \dot{G}\}$ as follows:
\begin{align*}
    \vartheta(\dot{\mathcal{U}}) := \ & (B^+, 1) \\
    \vartheta(\dot{\mathbb{1}}) := \ & (\{\mathbb{1}\}, 1) \\
    \vartheta(\dot{\sim}) := \ & (\sim_{\mathbb{B}^+}, 1) \\
    \vartheta(\dot{\leq}) := \ & (\leq_{\mathbb{B}^+}, 1) \\
    \vartheta(\dot{G}) := \ & (B^+, 0) \text{.}
\end{align*}
...
\begin{gather*}
    ...
    \ulcorner \forall p \ \forall q \ ((\dot{\leq}(p, q) \wedge \dot{G}(p)) \implies \dot{G}(q)) \urcorner \text{, and} \\
    \ulcorner \forall p \ \exists q \ (p = \dot{\mathbb{1}} \vee \dot{G}(p) \vee (\dot{G}(q) \wedge \dot{\sim}(p, q))) \urcorner \text{.}
\end{gather*} 
Let $\mathfrak{T} := (T, \sigma, \dot{\mathcal{U}}, \vartheta)$. Then $\mathfrak{T}$ is clearly a consistent $\Pi_2$ TCI, for any ultrafilter on $\mathbb{B}$ is an interpretation of $\dot{G}$ satisfying $\mathfrak{T}$. Further,
\begin{enumerate}[label=(\arabic*)]
    \item\label{4431} for each $p \in B^+$, $\{\ulcorner \dot{G}(p) \urcorner\} \in P(\mathfrak{T})$, and
    \item\label{4432} for each $x \in P(\mathfrak{T})$, 
    \begin{align*}
        \exists p \ \forall q \ (x \cup \{\ulcorner \dot{G}(p) \urcorner\} \in P(\mathfrak{T}) & \wedge (\ulcorner \dot{G}(q) \urcorner \in x \implies p \leq_{\mathbb{B}^+} q) \\
        & \wedge (\ulcorner \neg \dot{G}(q) \urcorner \in x \implies p \leq_{\mathbb{B}^+} \! (\sim_{\mathbb{B}^+} \! (q)))) \text{.}
    \end{align*}
\end{enumerate} 
Define $\pi : B^+ \longrightarrow P(\mathfrak{T})$ to be $$p \mapsto \{\ulcorner \dot{G}(p) \urcorner\},$$ which is possible by \ref{4431}. We argue that $\pi$ is a dense weak embedding from $\mathbb{B}^+_0$ into $\mathbb{P}(\mathfrak{T})$.

Denote $w(\leq_{\mathbb{P}(\mathfrak{T})})$ as $\leq^*$, and $w(\mathbb{P}(\mathfrak{T}))$ as $\mathbb{P}^*$. 

We first show that $\pi$ is a weak embedding. Since $\mathbb{B}^+_0$ is separative by Fact \ref{boolsep}, $\leq_{\mathbb{B}^+} \ = w(\leq_{\mathbb{B}^+})$. Assume $p \leq_{\mathbb{B}^+} q$ and let $x \leq_{\mathbb{P}(\mathfrak{T})} \pi(p)$. By the definition of $T$, $x \cup \pi(q) \in P(\mathfrak{T})$, so $x \not \! \! \! \bot_{\mathbb{P}(\mathfrak{T})} \ \pi(q)$. We thus have $\pi(p) \leq^* \pi(q)$. Next, assume $p \not \leq_{\mathbb{B}^+} q$. Then there is $r \leq_{\mathbb{B}^+} p$ such that $r \bot_{\mathbb{B}^+_0} \ q$, because $\mathbb{B}^+_0$ is separative. This means $(\pi(p) \cup \pi(r)) \ \bot_{\mathbb{P}(\mathfrak{T})} \ \pi(q)$, and $\pi(p) \not \leq^* \pi(q)$. Lastly, if $\pi(p) \not \! \! \! \bot_{\mathbb{P}^*} \ \pi(q)$, then $\pi(p) \not \! \! \! \bot_{\mathbb{P}(\mathfrak{T})} \ \pi(q)$, which guarantees $p \not \! \! \bot_{\mathbb{B}^+_0} \ q$.

To see that $ran(\pi)$ is dense in $\mathbb{P}^*$, fix any $x \in P(\mathfrak{T})$. By \ref{4432}, there is $p \in B^+$ for which
\begin{itemize}
    \item $x \cup \{\ulcorner \dot{G}(p) \urcorner\} \in P(\mathfrak{T})$,
    \item $\forall q \ (\ulcorner \dot{G}(q) \urcorner \in x \implies p \leq_{\mathbb{B}^+} q)$, and
    \item $\forall q \ (\ulcorner \neg \dot{G}(q) \urcorner \in x \implies p \leq_{\mathbb{B}^+} \! (\sim_{\mathbb{B}^+} \! (q)))$.
\end{itemize}
What this entails is, whenever $\mathcal{M} \models^* \mathfrak{T}$ and $\pi(p) \subset \Sigma(\mathcal{M})$, we must have $x \subset \Sigma(\mathcal{M})$. By the definition of $\mathbb{P}(\mathfrak{T})$ and the bijectivity of the function $\mathcal{M} \mapsto \Sigma(\mathcal{M})$, $\pi(p) \leq^* x$.
\end{proof}

From the perspective of forcing extensions, Theorems \ref{genericmodels} and \ref{revgenmodels}, in view of Remarks \ref{sepfor} and \ref{dwefor}, tell us that 
\begin{enumerate}
    \item every consistent $\Pi_2$ TCI has a model that ``generates'' a forcing extension, and
    \item every forcing extension is ``generated'' by a model of a consistent $\Pi_2$ TCI.
\end{enumerate}
This lends credence and weight to the informal thesis (a slogan, rather), $$\text{``Forcing is exactly } \Pi_2 \text{.''}$$ Further, it suggests one way to use a complexity class defined on TCIs as a measure of --- or a proxy for --- accessibility within the context of the set-theoretic multiverse. 

\subsection{More Generic Models of $\Pi_2$ TCIs}
...
However, the question dual to \ref{54qn2} appears much more difficult.

\begin{ques}\label{q548}
Is there a $\Pi_2$ TCI $\mathfrak{T}$ with a model $\mathcal{M}$ in some outer model of $V$ such that $\mathcal{M}$ is not isomorphic to any $V$-generic model of $\mathfrak{T}$?
\end{ques}
...
Is there a first-order $\Pi_2$ theory $T$ with a model $M$ in some outer model of $V$ such that for no $\Pi_2$ TCI $\mathfrak{T}$ is $M$ isomorphic to a $V$-generic model of $\mathfrak{T}$?
\end{ques}

The technique of forcing is apparently the only way we know how to extend an arbitrary model of set theory. By Lemma \ref{gmodelsinfe}, Questions \ref{q548} and \ref{q552} would be intractable using current technology if they were to be answered in the affirmative. We can modify them into possibly easier questions by only asking for consistency --- instead of outright existence --- over the base set theory.

\begin{ques}
\leavevmode
\begin{enumerate}[label=(\arabic*)]
    \item Is it consistent that there exists a $\Pi_2$ TCI $\mathfrak{T}$ with a model $\mathcal{M}$ in some outer model of $V$ such that $\mathcal{M}$ is not isomorphic to any $V$-generic model of $\mathfrak{T}$?
    \item Is is consistent that there exists a first-order $\Pi_2$ theory $T$ with a model $M$ in some outer model of $V$ such that for no $\Pi_2$ TCI $\mathfrak{T}$ is $M$ isomorphic to a $V$-generic model of $\mathfrak{T}$?
\end{enumerate}
\end{ques}

Our line of questioning can be extended to the paradigm of relative effectiveness.
...
We can modify Question \ref{q553} to get an analogue of Question \ref{q552} in the same spirit of relative effectiveness.

\begin{ques}
Is there a first-order $\Pi_2$ theory $T$ with a model $M$ such that whenever $\mathfrak{T}$ is a code-friendly $\Pi_2$ TCI and $\mathcal{M} \models^* \mathfrak{T}$,
\begin{enumerate}[label=(\alph*)]
